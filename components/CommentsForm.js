...
const CommentsForm = ({ slug }) => {
  ...
  //set form data to be stored if checkbox is checked
  ...
  //A function to submit post
  const handlePostSubmission = () => {
    setError(false);

    //throw error if text fields are not filled
    ...
    //store user data if checkbok is checked and remove data if otherwise
    if (storeData) {
    ...
    submitComment(commentObj)
      .then((res) => {
        if (res.createComment) {
          ...
          //briefly show a success message on form submittion
